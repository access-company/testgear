...
defmodule Testgear.ParamsValidationTest do
  ...
  test "in-process: should parse params and pass if the parameters are all valid" do
    res = ReqInProcess.post_json("/params_validation/1?foo=2", %{foo: 3}, %{"x-foo" => "4"}, cookie: %{"foo" => "5"})
  ...
  test "in-process: should pass validation if the list body is valid" do
    res = ReqInProcess.post("/list_body_validation", ~S([1, 2, 3]), %{"content-type" => "application/json"})
  ...
  test "in-process: should pass validation if the map body is valid" do
    res = ReqInProcess.post("/map_body_validation", ~S({"a": 1, "b": 2, "c": 3}), %{"content-type" => "application/json"})
